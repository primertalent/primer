...
- ⬜ **Build 3:** Deferred. Superseded by Phase 4 sliced (Gmail send only) as the next ingestion build.
...
- **Phase 4 sliced — Gmail send only (next build):** Gmail OAuth scoped to send only — not read, no calendar. Recruiter approves a draft in Wren; Wren sends it via their Gmail account. This is the approval-gated send path, delivered as a thin slice of Phase 4 before full Workspace OAuth. Build 3 (as previously scoped) and the LinkedIn extension are both deferred.
- **P4-3 (lower priority, open):** `intake_notes_ready` auto-upgrade on manual pipeline insert — fires only for manual "Add to a role" cases (auto-match creates pipeline at ingestion time, so this is narrow scope now). See COLLISION_AUDIT.md.
