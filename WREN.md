...
Brief divergence fix (commit 3c28f5c): the 9am email brief and the same-day in-app brief could differ in text and substance — a generate-once violation. Root cause was not a race: `cron-brief.js` resolved "newest conversation of any day" while `Wren.jsx` resolved "today's conversation" client-side by browser-local midnight (create-if-none), so whenever the 9am email preceded the first app open the two composed into DIFFERENT conversations and the then-conversation-scoped idempotency gate passed both (composed at different times → the later one saw inbound replies ingested after 9am → different substance). Fix: one shared `api/_lib/getOrCreateTodayConversation.js` (recruiter-scoped, recruiter-timezone day boundary, create-if-absent) used by BOTH callers; `api/compose-brief.js` now resolves the conversation server-side and returns conversation_id (the app defers instead of resolving client-side); composeBrief's idempotency gate widened conversation-scoped → recruiter-scoped (one morning_brief per recruiter per brief_date, any conversation) with a graceful 23505 re-read; backstop partial unique index on (recruiter_id, brief_date) for morning_brief rows (migration 20260714000001). Read-only client fallback so a failed compose never blanks the desk. composeBrief composition logic untouched.
Submittal closing posture (commit 3a09eca): submittal drafts were ending with "Worth 30 minutes?" — an ask-for-time CTA transplanted from the recruiter's outreach voice. Two sources: the shared `buildVoiceBlock` rule 7 (kept — correct for outreach) and the four external format specs in `submissionDraft.js`. Added a submittal-level `closingPosture` block that overrides rule 7 for HM-facing submittals, and rewrote all four format closes (bulleted/paragraph/concise/linkedin) from CTA energy to a confident handoff — candidate availability, offer of more detail, or nothing. `voiceRules.js` and the outreach prompts untouched.
Known edges (deliberately not fixed): brief conversation resolution uses recruiter.timezone while brief_date stays UTC — a narrow pre-existing boundary edge, left as-is. Post-9am inbound intentionally waits for tomorrow's brief or the tray; do NOT "fix" the brief to chase same-morning arrivals.
Session A (commit b8d1d18) and Session B (Meet transcript diagnosis; probe blocked on the Google client-secret rotation) are recorded under Session 38 below.

**Session 38 (2026-07-13) — Google read-scope expansion (Session A, shipped) + Meet transcript slice (Session B, diagnosed):**
Session A — read-scope expansion + calendar read (commit b8d1d18): expanded the OAuth consent to request gmail.readonly, calendar.events.readonly, and drive.meet.readonly alongside the existing gmail.send, so the recruiter re-consents once for all read scopes. Deduped the consent-URL builder into `src/lib/googleOAuth.js` (both GoogleConnectCard and the Wren.jsx inline hint import it — the scope string had drifted across two copies); access_type=offline + prompt=consent preserved (guarantees a refresh token covering the new scopes). New `recruiters.google_scopes` column (migration 20260713000001, nullable, no backfill) records the granted scope set; `google-auth.js` persists `tokens.scope` on every exchange and documents the refresh-token-nulling invariant at the write site. New `api/_lib/googleToken.js` (`getFreshAccessToken`) is the shared read-path token accessor (refresh + persist, never clears tokens); `gmail-send.js` untouched (pointer comment only) so the verified production send path stays byte-for-byte the same. New Tier 0 `list_calendar` tool in `wren.js`: checks google_scopes before any network call, reads primary-calendar events in the recruiter's timezone (local-midnight day boundaries, DST-correct — a post-review fix; the first cut anchored to UTC midnight and would have offset an Eastern "today" by the tz gap), bounded projection, offers connect on missing scope. `wrenAgent.js` gained the tool description + rule 7b. Proven in production by list_calendar; all four scopes granted and persisted.
Session B — Meet transcript read slice (diagnosed, not built): a `fetch_meet_transcripts` tool that finds Meet transcript Docs in Drive (drive.meet.readonly), exports their text, and hands it to the EXISTING intake machinery. Splice design approved: export `handleGeminiNotesPath` from `ingest-email.js` and call it from `wren.js` with transcript-derived args (title→subject, exported text→body, `meet:<driveFileId>`→message_id) — no parallel extraction pipeline. Autonomy inherited verbatim (Tier 0 candidate create + interaction, propose-only role match, pipeline via add_to_pipeline). Dedup reuses the `interactions.message_id` partial-unique index (`meet:<fileId>` key) — no migration. Approved refinements: `latest:true` convenience, optional `channel` param (interaction type='meeting_transcript'), one `ingestion_log` row per ingest via exported `writeIngestionLog`. BLOCKED: the Drive scope's real files.list/export behavior can't be settled by code inspection; the live probe (uses `getFreshAccessToken`; staged as `_meet_probe.mjs`, uncommitted) is blocked on the Google client secret not being available locally — rotation needed. Account-side prereq surfaced: transcripts exist only for the recruiter's own hosted Meet calls with transcription enabled (Workspace tier) — a beta-onboarding fact.

**Session 37 (2026-07-07) — D1 submittal signal fix + ingestion logging instrumentation:**
D1 submittal signal dropout (commit a0213f4): motivation and career signal existed on candidate records but never reached submittal drafts — both internal and external drafts returned [NEEDS: stated reason] even when captured. Two structural causes fixed: `career_signals` (which carries the motivation key) was selected in the candidate query but never rendered by `buildCandidateSection`, and the submittal path never queried the `debriefs` table. Now `buildCandidateSection` renders career_signals, and `buildSubmittalDraftPayload` fetches the 3 most recent debriefs (motivation_signals, risk_flags, summary — candidate+recruiter scoped, captured_at desc), threading both into internal and external surfaces. The format-toggle endpoint inherits the fix via the shared builder. Revision path untouched (signal enters at fresh generation; revisions preserve it). Rule zero held: empty signals still render nothing, [NEEDS] behavior unchanged. Both new blocks no-op cleanly on the pasted-resume path.
Complete ingestion logging (commit 5986eb1): `ingestion_log` recorded discards only, so a successfully processed inbound left no trace — "no log entry" was indistinguishable from never-arrived / classified-wrong / silent-error. Now every inbound writes exactly one row (success, discard, failure) via a log accumulator written once in a `finally` block — "one inbound, one row" is guaranteed by control flow, not by discipline across ~12 exit sites. Records outcome_path (canonical branch label), classification, matched entity type/ID, action_type, interaction_id, candidate_created, and an error column (fallback only — a branch that set its label then threw keeps the label). `handleGeminiNotesPath` takes the accumulator and sets its own fields. The log write is awaited-but-wrapped so a log failure can never 500 or block ingestion; the 401/405 pre-try exits stay unlogged (not authenticated inbound). Dedup hits log as duplicate_message_id. Zero behavior change to ingestion. Migration 20260707000001 adds the columns (verified applied via information_schema; post-deploy acceptance test confirmed a row writing with outcome_path populated). Instrumentation for the clean real-use week.
...
Brief email model A (commit 85ed2a6): the 9am brief now always emails, whether or not the recruiter saw it in-app. Removed the already_briefed skip from the cron email path; composeBrief's existing idempotency means the emailed brief is the same one seen in-app (generate-once-deliver-twice), or freshly composed if none existed. emailed_at remains the sole suppression (never twice, always at least once). The email is a delivery format, not a fallback — the reliable daily newsletter.
...
Sprint 2a — entity card primitive (commits af11344, 889186d, dc0bda0): CandidateCard, RoleCard, CompanyCard summoned into the thread via get_candidate, get_role, get_company. Computed insights per type (pipeline count, stage distribution, client objection history, placement history). Hardening: enrichment signals renamed to career_signals, CandidateCard fields expanded, entity-pull prose rule — card owns facts, prose compresses to the read + move.
Sprint 2b — canonical pipeline stages + move_stage (commits c9ce8fd, f250d67, d3e3620): stages.js canonical stage list + STAGE_WEIGHTS. Migration adds stage_reached, lost_reason, start_date, guarantee_days with CHECK constraints; 3 pre-submittal rows detached-and-kept. move_stage tool: writes pipeline_stage_history, handles bidirectional moves with backward-reason capture, correction-undo (clean vs dirty), terminal capture (placed with start_date, lost with lost_reason), add_to_pipeline writes first history row. Entity-pull prose discipline extended to all 3 cards; process_steps dropped from get_role payload.
Sprint 2c-1 — desk ticker rebuild (commit 8c7e81e): PIPELINE VALUE (weighted), IN PROCESS (inProcess++ above unknown-stage and weight-zero skips — submitted-stage deals count), SUBMITTALS THIS WEEK (pipeline_stage_history unique pipeline_id since Monday 00:00 local). AT RISK / NEXT MOVE removed; riskActions query dropped.
Stage-aware next-move suppression (commit 9baf418): suggestions default to silence; suppression clause leads, stage map follows as constraint on the rare warranted suggestion. Stage set explicit for sourced (no pipeline) through placed/lost.
set_comp + set_fee + ticker comp fix (commit aa7f6d2): set_comp writes deal-specific expected_comp (target 'pipeline') or role pay range comp_min/comp_max (target 'role'); single number writes both equal on role target. set_fee normalizes (n≥1 → n/100), sanity guard 0.05–0.60 with force bypass, announces from DB-returned values. Ticker comp resolution fixed: expected_comp (+high midpoint) → comp_min/max midpoint → target_comp_min/max midpoint.
...
- Motivation data dropout (session 31 bug): RESOLVED (commit a0213f4, Session 37). Root cause was structural, not the assumed notes-scan gap — career_signals was selected but never rendered, and the submittal path never queried debriefs. Both now thread into the drafts. See Session 37 above and FRICTION 6/3.
- get_role client-history uses the wrong timestamp column (Session-37 latent bug, FRICTION 7/7): the client objection-history debrief query in `wren.js` (~L828-830) selects and orders by `created_at`, but the canonical debriefs timestamp is `captured_at` (migration 20260418000002; runBackgroundDebrief reads/writes captured_at). `created_at` does not exist on debriefs, so the query errors silently and client objection history comes back empty — the screener loses objection context. Low blast radius, silent degradation of a moat input. Fix: `created_at` → `captured_at` in both the select and the order clause.
...
**Build plan (locked 2026-06-10):**

**Phase 0 — Burn the boats (this week)**
- Auth gate on `api/ai.js` (JWT, match `api/wren.js` pattern)
- Submit Google read-scope verification (Ryan, not code)
- Delete legacy surfaces (Desk.jsx, Candidates.jsx, CreateCandidate.jsx, EditCandidate.jsx, CandidateCard.jsx, CallMode.jsx, Roles.jsx, RoleDetail.jsx, EditRole.jsx, CreateRole.jsx, ActionCard.jsx, SidePanel.jsx, WrenCommand.jsx)
- `pipeline` → `pipelines` rename migration rides in the cleanup

**Phase 1 — The One Surface (two weeks, target ~6/26)**
- Wren speaks first: agent loop output composes the brief on `/wren` load
- Desk ticker: weighted value, at risk, next close
- Deal ledger artifact (stamped card, fee at stake, verdict pill, one named risk)
- Entity views become conversation renders, rebuilt as artifacts only when missed
- Moat bug fixes: shared fact base (screen output feeds submittal context), motivation scan of notes/interactions in draft builder
- Ambient pipeline v1: Gmail auto-forward to existing CloudMailin address (no Google review needed), ingestion event dispatcher (classify → resolve via matchWithConfidence → fire tool → land in brief)
- Injection defense designed into the dispatcher from line one: inbound content is data never instructions, hard-scoped tools per ingestion source, Tier 0/1 only for email-triggered actions, no outbound send triggered by inbound content without approval
- Exit test: Ryan runs his full desk for a week without wanting a deleted page back

**Phase 2 — The Pocket Employee (one week, target ~7/3)**
- PWA shell: manifest, service worker, install prompt
- Push wired to agent loop: 7am brief arrives as notification
- Voice input on composer (Web Speech API)
- Exit test: soccer test. Push → open → approve, under 10 seconds

**Phase 3 — The Memory That Compounds (two weeks, mid-July)**
- Outcome write-back: closing or losing a deal captures what happened and why, in conversation, 30 seconds
- Pattern retrieval: briefs and screens cite the recruiter's own history

**Phase 4 — Beta (parallel, not after)**
- 10 outreach conversations start week of 6/15, during Phase 1
- 5 users at $199 by late July; onboarding is pipeline paste
- Per-user token tracking ships before user five
- Week-one activation metric: first submittal drafted from the user's own real candidate within 48 hours of signup. Instrument it. Users who miss it get a call.

**Kill list (explicit, 2026-06-10)**
- Tier 2 chip wiring, "Add to a role" diagnosis, all Desk handler work: dead with the surfaces
- Gemini Notes email path polish: read-scope OAuth replaces it
- Granola/Fathom/texts/LinkedIn extension: not before five paying users
- Stage 9 cadence engine: two-day build AFTER outcome write-back exists, premature before
- Stage 8 full closing playbook: needs outcome data to be sharp; own-history lines in the brief are its v1 and ship in Phase 3

**Open operational items (not code)**
- Privacy policy + ToS with AI-output liability clause + data deletion path: before user five, ideally before July
- Supabase RLS audit: before user five
- One-click full data export: promised publicly on the pricing page. The rows export; the trained ladder, voice layer, and outcome patterns do not. The moat survives the export.
- Monthly one-hour competitive scan: watch Paraform building for its own network, sourcing startups drifting toward the desk.
