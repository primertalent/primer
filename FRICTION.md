...
6/3 | candidate_enrichment | RESOLVED (commit a0213f4 — buildCandidateSection renders career_signals; buildSubmittalDraftPayload queries recent debriefs). Motivation present in the candidate record but dropped from the submittal synthesis. The candidate-view surfaced "Nick is leaving due to organizational shifts and in-person office mandates, Unit is remote, clean motivation story," but the internal and external drafts both returned motivation as NOT CAPTURED / [NEEDS: stated reason]. Root cause was structural, not the assumed notes-scan gap: career_signals (which carries the motivation key) was selected but never rendered into the prompt, and the submittal path never queried the debriefs table (motivation_signals, risk_flags, summary). Fixed: both now thread into internal and external drafts — 3 most recent debriefs, candidate+recruiter scoped, captured_at desc. | missing_data
