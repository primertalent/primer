...
7/13 | ingestion | Phone calls leave no transcript to ingest. The Meet transcript slice (Session B) covers only hosted Google Meet calls with transcription on; phone screens — the bulk of a solo recruiter's candidate calls — produce no Drive artifact. Transcript ingestion is real coverage but partial: phone stays manual (paste/dictate) until a call-capture path (Granola/Fathom forwarding, or dictation) fills the gap. Not a Wren bug — a coverage boundary of the Google-only ingestion surface. | external_limitation

7/13 | agent | Silent mid-task stalls. Wren occasionally goes quiet mid-task — no output, no error, no completion signal; the turn appears to hang and is indistinguishable from a slow tool call while it's happening. Repro conditions not yet isolated. Need a visible working/stalled state (or turn-level instrumentation) so "thinking" is distinguishable from "died." | bug

7/13 | composer | Single-attachment composer. The composer takes one attachment at a time, so a candidate arriving as resume + separate JD or notes file needs multiple drops across turns. Source material usually arrives as a small bundle; multi-file attach (or multiple chips in one send) would match how the work actually shows up. | shape_problem

7/13 | brief | RESOLVED (config, no code). Brief email never sent since ship: CRON_SECRET was never created in Vercel, so cron requests carried no auth header and every hourly invocation 401'd at the guard since 6/25. Second layer: after creating the var, forgot the redeploy — env binds at deploy. Dry-run verified full path clean (guard, composeBrief, Anthropic call, HTML render, hour-bypass). Lesson: the 6/25 model-A "fix" redesigned delivery logic against a symptom whose root cause was config. When something never works, check the front door before redesigning the room. Watch 7/14 9am ET for real send. | bug

7/13 | ingestion | Person-role confusion: every name in an inbound surfaces as a potential candidate. Added the recruiter (self), hiring managers, and Paraform support as candidates. Classifier types the email (candidate_communication vs client_communication vs noise) but nothing types the person, so every extracted name lands in the only bucket that exists. Eventual fix shape: sender-domain heuristics (paraform.com never a candidate), recruiter's own address never creates a candidate, person-classification at extraction with propose-don't-write for ambiguous. Autonomy ladder applied to entity creation. | bug

7/13 | data | Clean-week reset executed. Full wipe of all fake/test data (72 candidates, 4 roles, 4 clients, 3 pipelines, 181 interactions, 15 debriefs, 136 actions, 23 conversations, 386 messages, 644 ingestion_log rows). Kept: recruiters row (OAuth tokens, timezone), voice_samples (was 0). Verified 16 zeros post-wipe. Desk at real zero; observation week runs on real work only from this line forward. | strategic-decision

7/7 | agent | OPEN. get_role's client-history debrief query (wren.js ~L828-830) selects and orders by created_at, but the canonical debriefs timestamp is captured_at (migration 20260418000002; runBackgroundDebrief reads/writes captured_at). created_at does not exist on debriefs — the query errors silently and client objection history comes back empty (recent_debriefs falls to []), so the screener loses objection context. Surfaced while mapping the D1 submittal fix (Session 37); left untouched per scope. Low blast radius but a silent degradation of a moat input. Fix: created_at → captured_at in both the select and the order clause. | bug
...
6/12 | ingestion | RESOLVED (commit 5986eb1 — one row per inbound, accumulator-in-finally). ingestion_log only recorded discards — a successfully processed inbound left no trace, so during debug "no log entry" was indistinguishable from "never arrived," "classified wrongly," or "silent error." Now every inbound writes exactly one row (success, discard, failure) via a log accumulator written once in a finally block; records outcome_path (canonical branch), classification, matched entity type/ID, action_type, interaction_id, candidate_created, and an error column. Dedup hits log as duplicate_message_id. Migration 20260707000001 adds the columns. Instrumentation for the clean real-use week. | missing_data
...
6/3 | candidate_enrichment | RESOLVED (commit a0213f4 — buildCandidateSection renders career_signals; buildSubmittalDraftPayload queries recent debriefs). Motivation present in the candidate record but dropped from the submittal synthesis. The candidate-view surfaced "Nick is leaving due to organizational shifts and in-person office mandates, Unit is remote, clean motivation story," but the internal and external drafts both returned motivation as NOT CAPTURED / [NEEDS: stated reason]. Root cause was structural, not the assumed notes-scan gap: career_signals (which carries the motivation key) was selected but never rendered into the prompt, and the submittal path never queried the debriefs table (motivation_signals, risk_flags, summary). Fixed: both now thread into internal and external drafts — 3 most recent debriefs, candidate+recruiter scoped, captured_at desc. Real-use validated 7/13 on the Nordwick submittal — captured motivation threaded into the draft, no [NEEDS: stated reason] regression. | missing_data
