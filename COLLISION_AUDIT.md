...
**P4-4 — "Add to a role" button reliability across all entry points. Reduced blast radius after P4-1.**
Auto-match eliminates the need for "Add to a role" on Gemini Notes cards when Pass 1 fires confidently. The button still matters for manual intake candidates and the proposed-match fallback path. Root cause of picker reliability still undiagnosed — likely state timing between `openRoles` load and picker render. Workaround: scroll to Pipeline section inside candidate page. Still open, lower priority than pre-P4-1.
